...
export function ResearchFinder() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await apiClient.findResearchOpportunities(profile);
      setResult(response);
    } catch (err: any) {
      setError(err.response?.data?.detail || 'Failed to find research opportunities. Please try again.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-gray-600">
            Discover research opportunities that match your interests
          </p>
        </div>
        ...
        <div className="card mb-8">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              <label className="label">CS Research Interests *</label>
              <input
                type="text"
                required
                className="input"
                placeholder="AI/ML, Robotics, Computer Vision, NLP"
